...
export async function onRequestGet({ request, env }) {
  ...
  if (!tokenResponse.ok) return errorResponse("Discord could not finish the login. Please try again.", 502);
  ...
  if (!joinResponse.ok) return errorResponse("Your Discord profile is ready, but the Support Server join could not be completed.", 502);
