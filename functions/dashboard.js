...
async function getDashboardServers(env, discordAccessToken, request) {
  ...
  try {
    ...
    const servers = await Promise.all(userGuilds.map(async (guild) => {
      ...
      const permissionBits = BigInt(String(guild.permissions || "0"));
      const isOwner = Boolean(guild.owner);
      const canManage = isOwner || (permissionBits & 0x20n) === 0x20n || (permissionBits & 0x8n) === 0x8n;
      return {
        id: String(guild.id),
        name: guild.name || "Discord server",
        owner: isOwner ? "Server owner" : (canManage ? "Server manager" : "Member"),
        isOwner,
        canManage,
        ...
        botLogs: syncedServers.get(String(guild.id))?.botLogs || [],
...
export async function onRequestGet({ request, env }) {
  ...
  const statsValues = [
    ["Members", Number(selectedServer?.members) || 0, "#ffc31c"],
    ["Bots", Number(selectedServer?.bots) || 0, "#67e84d"],
    ["Channels", Number(selectedServer?.channels) || 0, "#4ab8ff"],
    ["Roles", Number(selectedServer?.roles) || 0, "#b261ff"],
    ["Categories", Number(selectedServer?.categories) || 0, "#ff8a00"],
  ];
  const maxStatsValue = Math.max(1, ...statsValues.map(([, value]) => value));
  const statsBars = statsValues.map(([label, value, color]) => `
    <div class="stats-bar-row">
      <div class="stats-bar-label"><span>${escapeHtml(label)}</span><strong>${value.toLocaleString("en-US")}</strong></div>
      <div class="stats-bar-track"><i style="width:${Math.max(3, Math.round((value / maxStatsValue) * 100))}%;background:${color}"></i></div>
    </div>`).join("");
  const statsRows = statsValues.map(([label, value]) => `<tr><td>${escapeHtml(label)}</td><td>${value.toLocaleString("en-US")}</td><td><span class="stats-live-dot"></span>Live</td></tr>`).join("");
  const botLogs = Array.isArray(selectedServer?.botLogs) ? selectedServer.botLogs : [];
  const formatBotLogTime = (value) => {
    const timestamp = Date.parse(value || "");
    return Number.isFinite(timestamp) ? new Date(timestamp).toLocaleString("de-DE") : "Unbekannte Zeit";
  };
  const botLogRows = botLogs.length
    ? botLogs.map((entry) => `
              <article class="bot-log-entry">
                <span class="bot-log-dot" aria-hidden="true"></span>
                <div><div class="bot-log-head"><strong>${escapeHtml(entry.title)}</strong><time>${escapeHtml(formatBotLogTime(entry.at))}</time></div><p>${escapeHtml(entry.description || "Keine Beschreibung")}</p></div>
              </article>`).join("")
    : `<p class="bot-log-empty">Noch keine Bot-Aktionen synchronisiert. Sobald Beacon auf diesem Server etwas ausführt, erscheint es hier.</p>`;
  const historyPanel = `
        <section class="dash-content-section" id="dashboard-history" data-dashboard-section="dashboard-history">
          <div class="dash-panel history-panel">
            <div class="stats-heading"><div><span class="stats-eyebrow">BOT ACTIVITY</span><h2>Bot Logs</h2><p>Was Beacon auf diesem Server gemacht hat.</p></div><span class="history-live-badge"><span class="stats-live-dot"></span>Live sync</span></div>
            <div class="bot-log-list">${botLogRows}</div>
          </div>
        </section>`;
  const statisticsPanel = `
        <section class="dash-content-section" id="statistics" data-dashboard-section="statistics">
          <div class="dash-panel statistics-panel">
            <div class="stats-heading">
              <div><span class="stats-eyebrow">LIVE ANALYTICS</span><h2>Server Statistics</h2><p>Aktuelle Werte deines mit Beacon verbundenen Servers.</p></div>
              <button type="button" class="stats-refresh" data-stats-refresh>↻ Refresh</button>
            </div>
            <div class="stats-kpis">
              <article><span>Members</span><strong>${(Number(selectedServer?.members) || 0).toLocaleString("en-US")}</strong><small>Server total</small></article>
              <article><span>Bots</span><strong>${(Number(selectedServer?.bots) || 0).toLocaleString("en-US")}</strong><small>Detected bots</small></article>
              <article><span>Channels</span><strong>${(Number(selectedServer?.channels) || 0).toLocaleString("en-US")}</strong><small>All channels</small></article>
              <article><span>Roles</span><strong>${(Number(selectedServer?.roles) || 0).toLocaleString("en-US")}</strong><small>Configured roles</small></article>
            </div>
            <div class="stats-grid">
              <article class="stats-card stats-card--wide"><div class="stats-card-head"><h3>Server overview</h3><span>Live snapshot</span></div><div class="stats-bars">${statsBars}</div></article>
              <article class="stats-card"><div class="stats-card-head"><h3>Structure</h3><span>Current</span></div><div class="stats-donut"><div><strong>${(Number(selectedServer?.members) || 0).toLocaleString("en-US")}</strong><small>members</small></div></div><p class="stats-note">${Number(selectedServer?.categories) || 0} categories organize ${Number(selectedServer?.channels) || 0} channels.</p></article>
            </div>
            <div class="stats-table-wrap"><div class="stats-card-head"><h3>Tracked metrics</h3><button type="button" class="stats-export" data-stats-export>Copy CSV</button></div><table class="stats-table"><thead><tr><th>Metric</th><th>Value</th><th>Status</th></tr></thead><tbody>${statsRows}</tbody></table></div>
            <p class="stats-footnote">The dashboard reads the latest successful Beacon sync. Historical charts can be added once daily per-server snapshots are collected.</p>
          </div>
        </section>`;
  const simplePanels = [
  ...
  <head>
    ...
    <style>
      *{box-sizing:border-box}
      ...
      .dash-side-link--soon{padding-right:9px}
      .soon-badge{margin-left:auto;border:1px solid rgba(255,195,28,.5);border-radius:999px;background:rgba(255,195,28,.13);color:#ffc31c;padding:4px 7px;font-size:.65rem;font-weight:900;white-space:nowrap;box-shadow:0 0 14px rgba(255,195,28,.12)}
      ...
      .statistics-panel{max-width:1100px;background:#182332;border-color:rgba(255,255,255,.13)}
      .stats-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;border-bottom:1px solid rgba(255,255,255,.12);padding-bottom:20px}
      .stats-eyebrow{display:block;color:#ffc31c;font-size:.7rem;font-weight:900;letter-spacing:.16em}
      .stats-heading h2{margin:7px 0 4px;font-size:1.5rem}
      .stats-heading p{font-size:.85rem}
      .stats-refresh,.stats-export{border:1px solid rgba(255,195,28,.45);border-radius:7px;background:rgba(255,195,28,.12);color:#ffc31c;padding:9px 12px;font:800 .75rem "DM Sans",system-ui,sans-serif;cursor:pointer}
      .stats-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:20px}
      .stats-kpis article{border:1px solid rgba(255,255,255,.08);border-radius:9px;background:#101925;padding:16px}
      .stats-kpis span,.stats-kpis small{display:block;color:#8b95a6;font-size:.7rem;font-weight:900;letter-spacing:.1em;text-transform:uppercase}
      .stats-kpis strong{display:block;margin:9px 0 6px;color:#fff;font-size:1.7rem;line-height:1}
      .stats-kpis small{font-size:.72rem;letter-spacing:0;text-transform:none}
      .stats-grid{display:grid;grid-template-columns:1.35fr .65fr;gap:16px;margin-top:16px}
      .stats-card,.stats-table-wrap{border:1px solid rgba(255,255,255,.08);border-radius:9px;background:#101925;padding:18px}
      .stats-card-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:18px}
      .stats-card-head h3{margin:0;color:#fff;font-size:.95rem}
      .stats-card-head span{color:#8b95a6;font-size:.7rem;font-weight:900;text-transform:uppercase;letter-spacing:.1em}
      .stats-bar-row{margin:17px 0}
      .stats-bar-label{display:flex;justify-content:space-between;gap:12px;margin-bottom:7px;color:#c8cfdb;font-size:.8rem;font-weight:800}
      .stats-bar-label strong{color:#fff}
      .stats-bar-track{height:10px;overflow:hidden;border-radius:999px;background:#273244}
      .stats-bar-track i{display:block;height:100%;min-width:3%;border-radius:inherit;box-shadow:0 0 14px currentColor}
      .stats-donut{display:grid;width:155px;height:155px;margin:12px auto 18px;place-items:center;border-radius:50%;background:conic-gradient(#ffc31c 0 72%,#2e3b4e 72% 100%);position:relative}
      .stats-donut::after{content:"";position:absolute;inset:15px;border-radius:50%;background:#101925}
      .stats-donut div{position:relative;z-index:1;text-align:center}.stats-donut strong{display:block;color:#fff;font-size:1.55rem}.stats-donut small{color:#8b95a6;font-size:.72rem;font-weight:900;text-transform:uppercase}
      .stats-note{text-align:center!important;color:#9ea6b5!important;font-size:.8rem!important}
      .history-panel{max-width:1100px;background:#182332;border-color:rgba(255,255,255,.13)}
      .history-live-badge{display:flex;align-items:center;gap:7px;border:1px solid rgba(103,232,77,.35);border-radius:999px;background:rgba(103,232,77,.09);color:#67e84d;padding:8px 11px;font-size:.7rem;font-weight:900;white-space:nowrap}
      .bot-log-list{display:grid;gap:10px;margin-top:20px}
      .bot-log-entry{display:grid;grid-template-columns:10px minmax(0,1fr);gap:13px;border:1px solid rgba(255,255,255,.08);border-radius:9px;background:#101925;padding:14px 16px}
      .bot-log-dot{width:9px;height:9px;margin-top:5px;border-radius:50%;background:#ffc31c;box-shadow:0 0 12px rgba(255,195,28,.7)}
      .bot-log-head{display:flex;justify-content:space-between;align-items:baseline;gap:14px}
      .bot-log-head strong{color:#fff;font-size:.9rem}
      .bot-log-head time{color:#818b9d;font-size:.7rem;white-space:nowrap}
      .bot-log-entry p{margin:5px 0 0;color:#9ea6b5;font-size:.82rem;line-height:1.5}
      .bot-log-empty{border:1px dashed rgba(255,255,255,.15);border-radius:9px;padding:22px;color:#8b95a6!important;font-size:.85rem!important}
      .stats-table-wrap{margin-top:16px}.stats-table{width:100%;border-collapse:collapse;color:#d9deea;font-size:.82rem}.stats-table th{color:#8b95a6;font-size:.68rem;letter-spacing:.1em;text-align:left;text-transform:uppercase}.stats-table th,.stats-table td{border-bottom:1px solid rgba(255,255,255,.07);padding:12px 8px}.stats-table tbody tr:last-child td{border-bottom:0}.stats-table td:nth-child(2){color:#fff;font-weight:900}.stats-live-dot{display:inline-block;width:7px;height:7px;margin-right:6px;border-radius:50%;background:#67e84d;box-shadow:0 0 9px #67e84d}.stats-footnote{margin-top:16px!important;color:#778397!important;font-size:.75rem!important}
      ...
      .customize-panel--locked{position:relative;overflow:hidden}
      ...
      .dash-main.is-server-locked [data-dashboard-section="statistics"].is-active,.dash-main.is-server-locked [data-dashboard-section="badges"].is-active{display:block}
      ...
      @media (max-width:720px){.stats-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.stats-grid{grid-template-columns:1fr}.stats-heading{display:block}.stats-refresh{margin-top:14px}}
  ...
  <body>
    ...
    <script>
      ...
      function activateDashboardTab(id) {
        const publicTab = id === "statistics" || id === "badges";
        if (!serverSelected && !publicTab) {
      ...
      async function loadCurrentProfile() {
        ...
        try {
          const response = await fetch("/api/dashboard/customize?server=" + encodeURIComponent(dashboardServerId), { cache: "no-store" });
          const result = await response.json();
          if (!response.ok || !result.profile) return;
          const profile = result.profile;
          if (bioInput) bioInput.value = profile.bio || "";
          if (!avatarChanged && avatarPreview) { avatarValue = profile.avatarUrl || null; avatarApplied = true; avatarPreview.src = profile.avatarUrl || defaultImage; updateImageState(avatarPreview, avatarValue); }
          if (!bannerChanged && bannerPreview) { bannerValue = profile.bannerUrl || null; bannerApplied = true; bannerPreview.src = profile.bannerUrl || defaultImage; updateImageState(bannerPreview, bannerValue); }
        } catch (_) {}
      }
      try {
        const saved = JSON.parse(localStorage.getItem(customizeStorageKey) || "null");
        if (saved?.bio && bioInput) bioInput.value = saved.bio;
        if (saved?.avatar && avatarPreview) { avatarValue = saved.avatar; avatarApplied = saved.appliedAvatar === true; avatarRetryAt = Number(saved.avatarRetryAt) || 0; avatarChanged = !avatarApplied; avatarPreview.src = saved.avatar; updateImageState(avatarPreview, avatarValue); }
        if (saved?.banner && bannerPreview) { bannerValue = saved.banner; bannerApplied = saved.appliedBanner === true; bannerRetryAt = Number(saved.bannerRetryAt) || 0; bannerChanged = !bannerApplied; bannerPreview.src = saved.banner; updateImageState(bannerPreview, bannerValue); }
      } catch (_) {}
      loadCurrentProfile();
      ...
      async function saveBotChanges() {
        ...
        const payload = {};
        if (bioChanged) payload.bio = bioInput?.value || "";
        if (avatarChanged && Date.now() >= avatarRetryAt) payload.avatar = avatarValue;
        if (bannerChanged && Date.now() >= bannerRetryAt) payload.banner = bannerValue;
        if (!Object.keys(payload).length) return showDashboardToast("No changes to save");
        ...
        try {
          ...
          if (Array.isArray(result.skippedRateLimitedFields)) {
            if (result.skippedRateLimitedFields.includes("avatar")) { avatarChanged = true; avatarApplied = false; avatarRetryAt = Date.now() + 60_000; }
            if (result.skippedRateLimitedFields.includes("banner")) { bannerChanged = true; bannerApplied = false; bannerRetryAt = Date.now() + 60_000; }
          }
          ...
          try { localStorage.setItem(customizeStorageKey, JSON.stringify({ bio: bioInput?.value || "", avatar: avatarValue, banner: bannerValue, appliedAvatar: avatarApplied, appliedBanner: bannerApplied, avatarRetryAt, bannerRetryAt })); } catch (_) {}
      ...
      saveButton?.addEventListener("click", saveBotChanges);
      function scheduleAutoSave() {
        window.clearTimeout(autoSaveTimer);
        autoSaveTimer = window.setTimeout(() => saveBotChanges(), 450);
      }
      ...
      document.querySelectorAll(".asset-upload").forEach((input) => {
        input.addEventListener("change", () => {
          ...
          if (file.size > MAX_IMAGE_BYTES) {
            input.value = "";
            showDashboardToast("Images must be 5 MB or smaller", "error");
            return;
          }
      ...
      bioInput?.addEventListener("input", () => { bioChanged = true; scheduleAutoSave(); });
      ...
      document.querySelectorAll("[data-stats-refresh]").forEach((button) => {
        button.addEventListener("click", () => {
      ...
      document.querySelectorAll("[data-stats-export]").forEach((button) => {
        button.addEventListener("click", async () => {
          const rows = [...document.querySelectorAll(".stats-table tbody tr")].map((row) => [...row.cells].map((cell) => cell.textContent.trim().replace(/\s+/g, " ")));
          const csv = [["Metric", "Value", "Status"], ...rows].map((row) => row.map((cell) => '"' + cell.replaceAll('"', '""') + '"').join(",")).join("\\n");
          try {
            await navigator.clipboard.writeText(csv);
            showDashboardToast("CSV copied");
          } catch (_) {
            showDashboardToast("CSV could not be copied", true);
          }
        });
      });
